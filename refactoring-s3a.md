...
| 2019-04-05 | 0.1 beta | publish as gist |
...
Our re-referencing other top-level operations (`getFileStatus()` &c) was convenient, and nominally reduces cost, but it means the "it's a filesystem" metaphor is sustained deeper into the code than it needs to be. And the cost of those little `getFileStatus()` calls (2= HEAD + 1x LIST) is steep. Similarly, there are calls to `getObjectMetadata` &c which sneak in when you aren't paying attention. 
...
Initialization is fairly fragile. As the new extension classes (e.g. DelegationTokens) are passed in instances of S3AFileSystem during construction, they can call things before they are ready. It means reordering code there is dangerous.
...
To avoid the under layers becoming single large classes on their own, break up them by feature `MultiObjectDeleteSupport`, `CopySupport` which can be maintained & cherry picked independently. Benefits: better isolation of functionality. Weakness: what about if feature (a) needs feature (b)? You've just lost that isolation and added more complexity. Maybe we can do more at the S3AStore level, where features are given a ref to the S3ACore so can interact directly with that, along with the metastore
...
(Note: this has already been added in HADOOP-15625)

### `org.apache.hadoop.fs.s3a.impl.StoreBinding`

Those common services and values which need be shared all down the models
...
### `org.apache.hadoop.fs.s3a.impl.S3ACore`

`S3ACore` talks directly to S3 -and is the only place where we do this.
...
* new operations move to an async API, in anticipation of a move to async AWS SDK 2.0. But: need to look at that future SDK to make sure the work here is compatible.
...
* Contains references to `S3ACore` and the `S3Guard` Metastore.
...
the specific fields are passed down as contructor parameters, `StoreBinding` and, when appropriate,
...
* Include Source File status and Dest filestatus as optional fields.
If present, operations can use these rather than issue new requests for the data

* Include refs to the core statistics structures
...
* New S3ClientImpl which simulates base S3 API, so allow for may ITests to run locally as Tests with the right -D option on the CLI (or default if you don't have cluster bindings?)
...
Can we really do a clean separation of `S3ACore` operations from FileSystem model?
...
1. Create and evolve the `StoreBinding` class, use as constructor PM for new modules in the .impl package
