...
I think we are going to have adopt a radical release process of 90% automated monthly releases with the option of interim OOB security fixes. 
At the same time, the spread of the Sha Halud worm through the NPM package ecosystem shows us that a fully automated release process is deadly, especially when coupled with a build system where naively trusting the latest release is the usual dependency model.
